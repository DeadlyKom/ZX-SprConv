...
void FImage::ResizeRegion(const void* InImageData, ImVec2 OriginalSize, ImVec2 RequiredSize, ImVec2 uv0, ImVec2 uv1)
{
	uint32_t* ImageData = new uint32_t[uint32_t(RequiredSize.x * RequiredSize.y * 4)];
	if (stbir_resize_region((unsigned char*)InImageData, (int)OriginalSize.x, (int)OriginalSize.y, 0,
							(unsigned char*)ImageData,	 (int)RequiredSize.x, (int)RequiredSize.y, 0, 
							STBIR_TYPE_UINT8,
							4, 3, 0,
